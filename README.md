...
модель с учителем не видит вообще, а связка ловит больше трети.
...
стояло 42.7% — одиночный прогон, поданный как результат. Это было враньём по
неосторожности, и теперь все замеры канала усредняются по пяти зёрнам, а
разброс показывается рядом с медианой — и в отчёте, и на сайте.

**Чего это стоит.** Страховка не бесплатна. На знакомых схемах она снижает
PR-AUC с 0.933 до 0.917 и обходится примерно в 0.6 млн ₸ за тестовый период.
...
### 6. Контрфакты: что надо было сделать иначе
...
поиск границы, остальное — одиночные переключения.

Если смягчить нечем, система говорит и это — пустой список без объяснения
оператору бесполезен, он не понимает, искала система или нет.

Здесь же был показательный промах. Сначала поиск по сумме брал нижнюю границу
жёстко в 1% от исходной, и на честной командировке решение при этой сумме
не менялось. Система отвечала «сумма не помогает», а в README попала фраза,
что подтверждение дешевле пропуска **при любой сумме**. Это было неправдой:
после того как вилку стали искать делением пополам, граница нашлась — 1 300 ₸.
Красивое объяснение, построенное на непроверенной границе поиска, звучит
убедительно ровно до первой проверки.

### 7. Нагрузка на операторов
...
политика, назначающая сотни звонков в сутки, требует конкретного числа людей.
...
несут 42% всех сэкономленных денег, верхние 20% — 63%. Это и есть ответ на
нехватку людей: звонить надо по убыванию ожидаемой выгоды, а не по времени
поступления. Операция, где звонок экономит девятьсот тысяч, должна обойти ту,
где он экономит восемь, даже если пришла позже.

Ось кривой — доля звонков, а не число операторов. При нашем потоке один
человек закрывает всё, и кривая по операторам выродилась бы в две точки;
доля же показывает то, ради чего кривая нужна.

### 8. Мониторинг дрейфа

В ограничениях было записано, что модель обучена на одном месяце и в проде
нужен мониторинг. Эта строчка закрыта.
...
Воспроизвести: `python -m ml.drift`
...
### 5. Сколько система думает над операцией

Банку этот вопрос важен сразу после точности: авторизация карты укладывается
в сотни миллисекунд на весь путь, и антифрод — только часть этого пути.
Меряем не «предсказание модели», а полный путь, как он идёт в проде.

| Этап | p50 | p95 |
|---|---|---|
| Сборка признаков | 0.7 мс | 0.9 мс |
| Обе модели | 8.5 мс | 9.1 мс |
| Расчёт ожидаемых потерь | 0.07 мс | 0.10 мс |
| Объяснение (SHAP) | 1.7 мс | 2.2 мс |
| **Всего** | **11.1 мс** | **11.9 мс** |

Один процесс выдерживает около 320 000 операций в час. Месячный поток кейса —
100 000 операций, то есть примерно 140 в час.

Замер окупился сразу же. Сначала полный путь занимал 23.3 мс, и восемьдесят
процентов этого времени — 18.9 мс — уходило на обход Isolation Forest из
трёхсот деревьев. Проверка показала, что триста деревьев не покупают ничего:
ROC-AUC канала 0.930 против 0.934 у ста, PR-AUC в пределах шума. Сто деревьев
дали ту же медиану по незнакомым схемам и вдвое меньшую задержку. Без замера
по этапам это место так и осталось бы незамеченным — общее число выглядело
приемлемым.

Воспроизвести: `python -m ml.bench`
...
python -m ml.report            # абляция и эксперименты, ~40 секунд
...
ml/drift.py           мониторинг дрейфа: PSI и сдвиг калибровки
...
| GET | `/api/drift` | мониторинг дрейфа: PSI и сдвиг калибровки |
| GET | `/api/capacity` | нагрузка на операторов и концентрация выгоды в очереди |
| GET | `/api/bench` | замер задержки по этапам |
| GET | `/api/report` | абляция, подбор потолка, эксперимент с новой схемой |
...
не выстраиваются и таблицу невозможно читать по вертикали.
...
- **Модель** — что под капотом: схема тракта, настоящая кривая обучения с
  ...
  модель не видела, разброс по зёрнам, таблица подбора потолка страховки и
  разбивка задержки по этапам. Все числа приходят из `models/report.json` и
  `models/bench.json`, то есть из замеров, а не из текста.
...
- **Замеры канала аномалий шумные.** Разброс по зёрнам широкий, а колец в
  тестовом окне всего 32. Мы показываем медиану и границы, но доверять таким
  числам как точным нельзя — для этого нужен более длинный период.
...
- **Дрейф.** Мониторинг есть (`python -m ml.drift`), но переобучение по
  расписанию и автоматическая остановка при значимом сдвиге не реализованы —
  сейчас это отчёт для человека, а не контур управления.
...
| Пик памяти при сборке отчёта | 436 МБ |
| Память работающего сервиса | 322 МБ |
| Холодный старт приложения | 2.7 с |
| Сборка | pip ~2–4 мин, данные 5 с, обучение 25 с, отчёт 40 с |

Отчёт для вкладки «Модель» собирается тремя отдельными процессами: за один
отчёт обучается около десяти моделей, и в общем процессе их пик подбирается
к пределу тарифа. Отдельный процесс на шаг — и система забирает память
обратно после каждого. Если отчёт всё же не соберётся, деплой продолжится:
сайт умеет работать без него.
