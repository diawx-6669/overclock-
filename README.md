...
инженерии система отправила на звонок оператора **133**, а заблокировала
**ноль**. Из 193 краж карты — заблокировала **123**.
...
|---|---|
| ROC-AUC | 0.994 |
| PR-AUC | 0.892 (базовый уровень 0.020) |
| Полнота при точности 90% | 0.788 |
| Полнота при точности 75% | 0.842 |
| Brier после калибровки | 0.0048 |
| Угадан вид схемы | 98.6% |

| Что делает система | Значение |
|---|---|
| Отреагировала на фрод | 95.0% |
| Честных операций с трением | 4.01% |
| Заблокировано от всего потока | 0.98% |
| Потери за тестовый период | 7.2 млн ₸ |
| Без системы | 38.9 млн ₸ |
| С лучшим фиксированным порогом | 13.2 млн ₸ |

**Экономия против фиксированного порога — 6.0 млн ₸ за 25 000 операций.**
...
|---|---|---|---|
| Украденная карта | 193 | 98.4% | блокировка 123, подтверждение 47, звонок 20 |
| Обман клиента | 160 | 87.5% | **звонок 133**, подтверждение 7, блокировок 0 |
| Кольцо карт | 148 | 98.6% | блокировка 121, звонок 16, подтверждение 9 |

Социальная инженерия ловится хуже остальных — 87.5%. Так и должно быть: по
...
```bash
make test        # 51 проверка
```

Обучение воспроизводимо до последнего знака: генератор и LightGBM работают с
фиксированными сидами, многопоточная недетерминированность отключена флагами
`deterministic` и `force_row_wise`. Два запуска подряд дают одинаковые метрики.
...
- **Социальная инженерия ловится хуже всего** — 87.5%. Это предел того, что
