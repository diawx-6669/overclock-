...
Всё это воспроизводится пятью командами и показано на сайте во вкладке
...
3 000 клиентов, 100 000 транзакций, 1.79% фрода. Все числа выше — про эти
данные; настоящий фрод разнообразнее и меняется каждый месяц.
...
Социальная инженерия ловится хуже остальных схем — 88.8% в медиане по
складкам. Это предел того, что видно по транзакционным данным: по формальным
признакам такая операция неотличима от обычной. Дальше нужны сигналы с
устройства — активные сессии удалённого доступа, длительность разговора,
поведение в приложении. Мы считаем честным показать этот разрыв, а не прятать
его за средним числом.

Матрицу эффективности действий в проде нужно набирать A/B-тестами. Замер
устойчивости показывает, что ошибка в четверть почти не влияет на выбор
действия, но измерить её всё равно стоит — это первое, что имеет смысл сделать
после запуска.
